...
if selected == "Predictions":
    ...
    try:
        with st.form("form1"):
            ...
            max_date=date.today()
            lease_commence_date =st.date_input('Lease Commence Date',min_value=min_date,max_value=max_date)
            col1,col2,col3=st.columns([1,0.1,1])
            with col1:
                stval=st.number_input("Enter the minimum storey Range",format="%d",step=1)
            with col2:
                st.write("TO")
            with col3:
                spval=st.number_input("Enter the maximum storey Range",format="%d",step=1)
        ...
        st.write("Enter the Input Values Accordingly")
